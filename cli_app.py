...
print('ENCRYPTOR\n')
print('1. Symmetric\n2. Asymmetric')
encryption_mode_number = input('\nNumber: ')
# Symmetric
if encryption_mode_number == '1':
    message = input('\nMessage: ')
    password = getpass()
    ...
    # Symmetric encrypt
        ...
        encryption_key = encryptor.create_encryption_key(
            int(settings['argon2']['time_cost']),
            int(settings['argon2']['memory_cost']),
            int(settings['argon2']['parallelism']),
            password,
            salt,
        )
        iv = encryptor.generate_iv()
        ciphertext = encryptor.encrypt_text(message, encryption_key, iv)
        config = encryptor.create_symm_config(settings, salt, iv, ciphertext)
        print(f'\nEncrypted message: {config}')
# Asymmetric
elif encryption_mode_number == '2':
    print('\n1. Create room\n2. Confirm room\n3. Open room\n')
    action = input('\nNumber: ')
    # Create room
    if action == '1':
        ...
        salt = encryptor.generate_salt()
        config = encryptor.create_asymm_config(g, p, public_key, salt)
