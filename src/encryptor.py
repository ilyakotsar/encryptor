...
def symmetric():
    if request.method == 'POST':
        ...
        if 'encrypt' in data:
            ...
            iv = generate_iv()
            ciphertext = encrypt_text(plaintext, encryption_key, iv)
            encrypted_message = create_symm_config(
                time_cost,
                memory_cost,
                parallelism,
                salt,
            ...
            password = data['password']
            config = symm_config_str_to_dict(encrypted_message)
            encryption_key = create_encryption_key(
                config['time_cost'],
                config['memory_cost'],
                config['parallelism'],
            ...
            plaintext = decrypt_text(config['ciphertext'], encryption_key, config['iv'])
            return jsonify({
                'plaintext': plaintext,
